...
The **ForAll** and **Map** functions evaluates a formula for all the records of input tables. The formula calculates a value, returning the result for each record processed in a result table. 

**ForAll** can also perform actions, such as modifying data or working with a connection. Use the [**With** function](function-with.md) to evaluate the formula for a single record.

**Map** can walk multiple tables at once and combine their values. 

Use the [**Sequence** function](function-sequence.md) with these functions to iterate based on a count.
...
**ForAll** and **Map** differ in their treatment of errors. If the result is an error value, **ForAll** will return a single error instead of the table, while **Map** will place the error in the result table. 
...
```powerapps-dot
Map( Books As Book, 
     Sequence(1000) As Sequence, 
     Patch( Book, {Index: Sequence.Value} ),
     MapLength.First )
```

Use **Map** to combine a table of data with an index table generated by the **Sequence** function. The **Patch** function is used here to add an addition column to the existing record. **MapLength.First** is used so that we know we have every book, even if the result of the **Sequence** runs out before the end of the Books table.

This can be useful with the Power Apps **Gallery** control to number items in a scrolling list.

#### Working with JSON

Let's take our Product table from before, but this time let's have it in a JSON text string. **Map** and **ForAll** can operate on Dynamic values:
...
Map( ParseJSON( "
     ...
     " ),
     { Product: Text(ThisRecord.Product), 
       'Quantity to Order': 
            Max( ThisRecord.'Quantity Requested' - ThisRecord.'Quantity Available', 
                 0) 
     }
)
```
...
`[{Product:"Widget", Quantity to Order:3}, {Product:"Gadget", Quantity to Order:0}, {Product:"Gizmo", Quantity to Order:0}, {Product:"Apparatus", Quantity to Order:1}]`

**Map** and **ForAll** can iterate over a top level array in the JSON, treating each element of the array as `ThisRecord`. Values are Dynamic until typed, which requires the **Text** function for `ThisRecord.Product`, but the type is implied from usage as a number with the subraciton operator `-` for `'Quantity Requested'` and `'Quantitiy Available'`.

The JSON array need not be homogenous. The **IsBlank** and **IsType** functions can be used to detect what fields are available in `ThisRecord`. For example:

```powerapps-dot
Map( ParseJSON( "
      [
        { ""Product"": ""Widget"", ""Size"": ""XL"" },
        { ""Product"": ""Apparatus"", ""Details"": { ""Liters"": 100, ""Warnings"": ""Toxic"" } },
        { ""Product"": ""Gizmo"" }
      ]
     " ),
     { Product: Text(ThisRecord.Product), 
       'Notes': 
            Concat( 
                Filter( [
                    If( !IsBlank(ThisRecord.Size), "Size: " & Text(ThisRecord.Size) ),
                    If( !IsBlank(ThisRecord.Details.Liters), "Size (Liters): " & Text(ThisRecord.Details.Liters) ),
                    If( !IsBlank(ThisRecord.Details.Warnings), "Warnings: " & Text(ThisRecord.Details.Warnings) ) 
                ], 
                !IsBlank(Value) ), 
            Value, ", ")
     }
...
`[{Notes:"Size: XL", Product:"Widget"}, {Notes:"Size (Liters): 100, Warnings: Toxic", Product:"Apparatus"}, {Notes:"", Product:"Gizmo"}]`
