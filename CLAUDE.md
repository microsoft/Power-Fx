...
Power Fx functions that interact with LLMs (like Copilot) externalize their prompt templates to resource files for security and auditability.

**Location:** `src/strings/PowerFxResources.*.resx` (keys prefixed with `FunctionNamePrompt...`)

**Security Guidance:**
- Prompt changes should be reviewed for injection risks (see OWASP LLM Top 10)
- Use structured formatting (JSON, XML) to separate system/user content
- Avoid dynamic string construction from user input in prompts
- Test prompt changes with adversarial inputs

**Pattern:**
```csharp
var template = StringResources.Get("FunctionPromptTemplate")
    ?? "fallback value";  // Always provide fallback
```
