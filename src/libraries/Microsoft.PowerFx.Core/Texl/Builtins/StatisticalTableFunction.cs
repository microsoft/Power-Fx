    ...
    internal abstract class StatisticalTableFunction : FunctionWithTableInput
    {
        ...
        public override bool IsServerDelegatable(CallNode callNode, TexlBinding binding)
        {
            ...
            || ExpressionContainsView(callNode, binding))
            {
                ...
                {
                    TrackingProvider.Instance.SetDelegationTrackerStatus(DelegationStatus.NotANumberArgType, callNode, binding, this, DelegationTelemetryInfo.CreateUnsupportArgTelmetryInfo(binding.GetType(args[1])));
                }
                else
                {
                    TrackingProvider.Instance.SetDelegationTrackerStatus(DelegationStatus.InvalidArgType, callNode, binding, this, DelegationTelemetryInfo.CreateUnsupportArgTelmetryInfo(binding.GetType(args[1])));
                }
