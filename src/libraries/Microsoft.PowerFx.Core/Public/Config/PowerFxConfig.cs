    ...
    public sealed class PowerFxConfig
    {
        ...
        internal void AddFunction(TexlFunction function)
        {
            if (function.HasLambdas || function.HasColumnIdentifiers)
            {
                ...
                {
                    if (function.HasLambdas && function.HasColumnIdentifiers && function.IsLambdaParam(i) && function.ParameterCanBeIdentifier(i))
                    {
                ...
                if (overloads.Any())
                {
                    for (var i = 0; i < Math.Min(function.MaxArity, 20); i++)
                    {
                        if ((function.IsLambdaParam(i) && overloads.Any(ov => ov.HasColumnIdentifiers && ov.ParameterCanBeIdentifier(i))) ||
                            (function.ParameterCanBeIdentifier(i) && overloads.Any(ov => ov.HasLambdas && ov.IsLambdaParam(i))))
                        {
                            (var message, var _) = ErrorUtils.GetLocalizedErrorContent(TexlStrings.ErrInvalidFunction, null, out var errorResource);
                            throw new ArgumentException(message);
                        }
