...
#pragma warning restore SA1313
        ...
        internal class JsonProcessing
        {
            ...
            internal FormulaValue Process()
            {
                ...
                string json = JsonInternal(_args[0], 0);
            ...
            {
                _flags = new JsonFlags() { HasMedia = JsonFunction.DataHasMedia(_args[0].Type._type) };
                ...
                _flags.Compact = !_flags.IndentFour;

                if ((_flags.IncludeBinaryData && _flags.IgnoreBinaryData) ||
                    (_flags.HasMedia && !_flags.IncludeBinaryData && !_flags.IgnoreBinaryData) ||
                    (!_flags.IgnoreUnsupportedTypes && JsonFunction.HasUnsupportedType(_args[0].Type._type, out var _, out var _)))
                {
                    _flags = null;
                    return;
                }
...
#pragma warning disable CS0618 // Use of [Obsolete] member: Date[Time]Value.Value
            ...
            internal string JsonInternal(FormulaValue arg0, int indent)
            {
                return arg0 switch
                {
                    ...
                    DateValue dv => WithQuotes(TimeZoneInfo.ConvertTimeToUtc(dv.Value, _timeZoneInfo).ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)),
                    DecimalValue dmv => dmv.Value.ToString(CultureInfo.InvariantCulture),
                    GuidValue gv => WithQuotes(gv.Value.ToString("D", CultureInfo.InvariantCulture)),
                    NumberValue nv => nv.Value.ToString(CultureInfo.InvariantCulture),
                    OptionSetValue osv => GetOptionSetValueString(osv),
                    RecordValue rv => GetJsonStringForRecord(rv, indent),
...
#pragma warning restore CS0618
            ...
            private static string WithQuotes(string str) => $@"""{str}""";

            internal static string GetColorString(Color color) => WithQuotes($"#{color.R:x2}{color.G:x2}{color.B:x2}{color.A:x2}");

            internal static string GetOptionSetValueString(OptionSetValue optionSetValue)
            {
                if (optionSetValue.Type._type.OptionSetInfo is EnumSymbol enumSymbol)
                {
                    ...
                    return WithQuotes(optionSetValue.ExecutionValue.ToString());
                }
                else if (optionSetValue.Type._type.OptionSetInfo is IExternalOptionSet ops)
                {
                    return WithQuotes(optionSetValue.Option);
                }

                return null;
            }

            internal string GetJsonStringForRecord(RecordValue recordValue, int indent)
            {
                StringBuilder stringBuilder = new StringBuilder();
                bool b = false;

                AddIndentation(stringBuilder, indent, false);
            ...
            internal string GetJsonStringForTable(TableValue tableValue, int indent)
            {
                StringBuilder stringBuilder = new StringBuilder();
                bool b = false;
                ...
                foreach (DValue<RecordValue> row in tableValue.Rows)
                {
                    ...
                    stringBuilder.Append(JsonInternal(row.Value, indent));
                    b = true;
                }
            ...
            {
                if (_flags.IndentFour)
                {
                    stringBuilder.Append(Environment.NewLine);
                }
            }

            private void AddIndentation(StringBuilder stringBuilder, int indent, bool newLine = true)
            {
                if (newLine)
                {
                    AddNewline(stringBuilder);
