    ...
    internal static partial class Library
    {
                ...
                if (res.IsError)
                {
                    ...
                    foreach (var error in res.Error.Errors)
                    {
                        ...
                        var messageProperty = new NamedValue(
                            "Message",
                            FormulaValue.New(error.GetMessageInLocale(runner.CultureInfo)));
        ...
        public static FormulaValue Error(IRContext irContext, FormulaValue[] args)
        {
            ...
            foreach (var errorRecord in errorRecords)
            {
                ...
                if (errorRecord.GetField(ErrorType.MessageFieldName) is StringValue messageField)
                {
                    result.Add(new ExpressionError { Kind = errorKind, Message = messageField.Value });
                }
                else
                {
                    result.Add(GetDefaultErrorMessage(errorKind));
                }
            }
        ...
        private static ExpressionError GetDefaultErrorMessage(ErrorKind errorKind)
        {
            switch (errorKind)
            {
                case ErrorKind.None:
                    // Default message that is shown to users when an error was produced, but with kind = 'none'
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrSystemNoError };
                case ErrorKind.Sync:
                case ErrorKind.Unknown:
                case ErrorKind.Internal:
                    // Default message that is shown to users when a system error was produced
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrSystemError };
                case ErrorKind.MissingRequired:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrMissingRequiredField };
                case ErrorKind.CreatePermission:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrCreateRecordPermissionDenied };
                case ErrorKind.EditPermissions:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrUpdateRecordPermissionDenied };
                case ErrorKind.DeletePermissions:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrDeleteRecordPermissionDenied };
                case ErrorKind.GeneratedValue:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrColumnServerGenerated };
                case ErrorKind.Conflict:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrRecordUpdateConflict };
                case ErrorKind.NotFound:
                    // Default message that is shown to users when they try to access a record that does not exist
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrRecordNotFound };
                case ErrorKind.ConstraintViolated:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrValidationError };
                case ErrorKind.ReadOnlyValue:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrColumnReadOnly };
                ...
                case ErrorKind.BadRegex:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrRegexSyntaxError };
                case ErrorKind.InvalidFunctionUsage:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrInvalidFunctionUsage };
                case ErrorKind.FileNotFound:
                    // Default message that is shown to users when they try to access a file that does not exist
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrFileNotFound };
                case ErrorKind.AnalysisError:
                    // Default message that is shown to users when they encounter an analysis error from Power Apps
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrSystemAnalysisError };
                case ErrorKind.ReadPermission:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrReadRecordPermissionDenied };
                case ErrorKind.NotSupported:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrOperationNotSupportedByPlayerDevice };
                case ErrorKind.InsufficientMemory:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrInsufficientMemoryStorage };
                case ErrorKind.QuotaExceeded:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrStorageQuotaExceeded };
                case ErrorKind.Network:
                    // Default message that is shown to users when they receive an error over the network
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrNetworkError };
                case ErrorKind.Numeric:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrNumericError };
                case ErrorKind.InvalidArgument:
                    // Default message that is shown to users when they pass an invalid argument to a function
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrInvalidArgument };
                case ErrorKind.NotApplicable:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrNotApplicable };
                case ErrorKind.Timeout:
                    // Default message that is shown to users when they execute an operation that was cancelled because of a timeout
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrTimeoutError };
                case ErrorKind.ServiceUnavailable:
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrOnlineServiceNotAvailable };
                case ErrorKind.InvalidJSON:
                    // Default message that is shown to users when they receive a JSON result that is invalid 
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrInvalidJsonFormat };
                case ErrorKind.Custom:
                    // Default message that is shown to users when they create an error with a custom kind
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrCustomError };
                default:
                    ...
                    if (intKind < 1000)
                    {
                        ...
                        return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrReservedErrorArg, MessageArgs = new object[] { intKind } };
                    }
                    ...
                    return new ExpressionError { Kind = errorKind, ResourceKey = RuntimeStringResources.ErrCustomErrorArg, MessageArgs = new object[] { intKind } };
            }
        }
