...
namespace Microsoft.PowerFx.Interpreter.Tests
{
    public class ConfigTests
    {
        ...
        public async Task BasicDirectEvalFunc()
        {
            ...
            // Per expression.
            var engine = new RecalcEngine();
            var result = await engine.EvalAsync("Func1(3)", CancellationToken.None, symbolTable: s1);
        ...
        public async Task TopLevelVariableDisplayName(string expression, string expectedInvariantExpression, string expectedDisplayExpression, double expected)
        {
        ...
        public async Task Shared()
        {
            ...
            var result3 = await engine.EvalAsync(expr, CancellationToken.None, symbolTable: s3Common); // 1*2 & 2*3  = "26"
            Assert.Equal("26", result3.ToObject());

            var result4 = await engine.EvalAsync(expr, CancellationToken.None, symbolTable: s4Common); // 1*2 & 2*4  = "28"            
            Assert.Equal("28", result4.ToObject());
        }

        [Fact]
        public async Task RecalcEngine_Symbol_CultureInfo()
        {
            ...
            Assert.Equal(1.0m, (await engine.EvalAsync("1.0", CancellationToken.None, options: us_ParserOptions, runtimeConfig: us_Symbols)).ToObject());
            await Assert.ThrowsAsync<InvalidOperationException>(async () => await engine.EvalAsync("1.0", CancellationToken.None, options: fr_ParserOptions, runtimeConfig: fr_Symbols));
            await Assert.ThrowsAsync<InvalidOperationException>(async () => await engine.EvalAsync("2,0", CancellationToken.None, options: us_ParserOptions, runtimeConfig: us_Symbols));
            Assert.Equal(2.0m, (await engine.EvalAsync("2,0", CancellationToken.None, options: fr_ParserOptions, runtimeConfig: fr_Symbols)).ToObject());

            // With .Net 5.0+, Persian decimal representation has changed
#if NET7_0_OR_GREATER
            // Note, the comma is 0x066B - Arabic Decimal Separator.
            Assert.Equal("2٫01", (await engine.EvalAsync("Text(2,01)", CancellationToken.None, options: fr_ParserOptions, runtimeConfig: fa_Symbols)).ToObject());
#else
            Assert.Equal("2/01", (await engine.EvalAsync("Text(2,01)", CancellationToken.None, options: fr_ParserOptions, runtimeConfig: fa_Symbols)).ToObject());
#endif
        }
        ...
        public async Task LocalFunctionDirect()
        {
            ...
            var result = await engine.EvalAsync("User(3)", CancellationToken.None, symbolTable: s1, runtimeConfig: runtime);
        ...
        public async Task LocalFunctionWithParameters()
        {
            ...
            // Pass RuntimeConfig directly through eval.
            var result = await engine.EvalAsync("User(x)", CancellationToken.None, symbolTable: s1, runtimeConfig: r12);
