...
";
        ...
        public async Task ParseAll()
        {
            ...
            // var dir = @"D:\dev\pa2\PowerApps-Client\src\AppMagic\WadlConverterSrc\WadlConverter\WadlConverter.Tests\SwaggerFiles";
            var dir = @"C:\temp\connectors\Tests";
            foreach (var file in Directory.EnumerateFiles(dir, "*.json"))
            {
                try
                {
                    var apiDoc = ReadSwaggerFromFile(file);
        ...
        public async Task Test2()
        {
            ...
            var config = new PowerFxConfig();
            var stream = await httpClient.GetStreamAsync(@"https://localhost:44393/swagger/v1/swaggerV2.json");

            var result = await new OpenApiStreamReader().ReadAsync(stream);
            var apiDoc = result.OpenApiDocument;
            
            config.AddService("Test", apiDoc, httpClient);

            var engine = new RecalcEngine(config);

            //var r = await engine.EvalAsync("Test.Test2({ x1 : 3 })", CancellationToken.None);

            var r = await engine.EvalAsync("Test.TestPost(13)", CancellationToken.None);
        }
#endif
    }
}
