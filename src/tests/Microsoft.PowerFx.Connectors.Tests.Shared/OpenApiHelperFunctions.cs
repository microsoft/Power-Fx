    ...
    internal static class OpenApiHelperFunctions
    {
            ...
            where T : FormulaValueSerializer
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    await jsonSerializer.SerializeValueAsync(parameter.Key, parameter.Value.Schema == null ? null : new ConnectorApiSchema(parameter.Value.Schema), parameter.Value.Value);
                }
            }
